...
describe('SiteSettingsService', () => {
    it('returns stored settings', async () => {
        ...
        const service = new SiteSettingsService({ select: jest.fn().mockReturnValue(query([settings])) } as never)
        await expect(service.get()).resolves.toEqual(settings)
    })

    it('returns defaults when settings do not exist', async () => {
        const service = new SiteSettingsService({ select: jest.fn().mockReturnValue(query([])) } as never)
        await expect(service.get()).resolves.toEqual(expect.objectContaining({ id: null, siteName: 'My App', maintenanceMode: false }))
    })
    ...
    it('updates existing settings', async () => {
        ...
        expect(update.set).toHaveBeenCalledWith(expect.objectContaining({ siteName: 'New', updatedAt: expect.any(Date) }))
    })
    ...
    it('creates initial settings when none exist', async () => {
        ...
        const db = { select: jest.fn().mockReturnValue(query([])), insert: jest.fn().mockReturnValue(insert) }
        const service = new SiteSettingsService(db as never)

        await expect(service.update({ siteName: 'New' } as never)).resolves.toEqual(created)
        expect(insert.values).toHaveBeenCalledWith(expect.objectContaining({ siteName: 'New', maintenanceMode: false }))
    })
})
