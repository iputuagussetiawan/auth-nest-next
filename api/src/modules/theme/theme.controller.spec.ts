...
describe('ThemeController', () => {
    ...
    it('wraps public and preference endpoints', async () => {
        ...
        await expect(controller.getMyTheme({ user: { userId: 'user-id' } })).resolves.toEqual({
            status: 'success',
            message: 'My theme',
            data: { id: 'theme-id' },
        })
        await expect(controller.listPublic()).resolves.toEqual({
            status: 'success',
            message: 'Themes',
            data: [{ id: 'theme-id' }],
        })
        await expect(controller.getActive()).resolves.toEqual({
            status: 'success',
            message: 'Active theme',
            data: { id: 'theme-id' },
        })
        await expect(
            controller.setPreference({ user: { userId: 'user-id' } }, { themeId: 'theme-id' }),
        ).resolves.toEqual({
            status: 'success',
    ...
    it('wraps admin theme operations', async () => {
        ...
        await expect(controller.create({ name: 'Ocean' } as never)).resolves.toEqual({
            status: 'success',
            message: 'Theme created',
            data: { id: 'theme-id' },
        })
        await expect(controller.activate('theme-id')).resolves.toEqual({
            status: 'success',
            message: 'Theme activated',
            data: { id: 'theme-id', isActive: true },
        })
        await expect(controller.update('theme-id', {} as never)).resolves.toEqual({
            status: 'success',
            message: 'Theme updated',
            data: { id: 'theme-id' },
        })
        await expect(controller.delete('theme-id')).resolves.toEqual({
            status: 'success',
