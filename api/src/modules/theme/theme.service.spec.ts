...
describe('ThemeService', () => {
    it('creates themes and deactivates existing themes when active', async () => {
        ...
        await expect(
            service.create({ name: 'Ocean', slug: 'ocean', isActive: true, config: {} } as never),
        ).resolves.toEqual({ id: 'theme-id', name: 'Ocean' })
    ...
    it('sets a valid preference and rejects an unknown theme', async () => {
        ...
        const db = {
            select: jest.fn().mockReturnValue(query([{ id: 'theme-id' }])),
            update: jest.fn().mockReturnValue(update),
        }
        const service = new ThemeService(db as never)

        await expect(service.setPreference('user-id', 'theme-id')).resolves.toEqual({
            themeId: 'theme-id',
        })
        expect(update.set).toHaveBeenCalledWith(
            expect.objectContaining({ preferredThemeId: 'theme-id', updatedAt: expect.any(Date) }),
        )

        const missing = new ThemeService({ select: jest.fn().mockReturnValue(query([])) } as never)
        await expect(missing.setPreference('user-id', 'missing')).rejects.toBeInstanceOf(
            NotFoundException,
        )
    })
    ...
    it('activates and updates themes', async () => {
        ...
        await expect(service.update('theme-id', { name: 'New' } as never)).resolves.toEqual(
            activated,
        )
        expect(db.update).toHaveBeenCalled()
        ...
        await expect(missing.update('missing', {} as never)).rejects.toBeInstanceOf(
            NotFoundException,
        )
    })
