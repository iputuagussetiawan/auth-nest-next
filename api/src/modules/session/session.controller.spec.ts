...
describe('SessionController', () => {
    ...
    it('forwards session requests and wraps responses', async () => {
        ...
        await expect(controller.getSessions(req as never)).resolves.toEqual({ status: 'success', message: 'Sessions fetched', data: [{ id: 'current' }] })
        await expect(controller.getOtherSessions(req as never)).resolves.toEqual({ status: 'success', message: 'Sessions fetched', data: [{ id: 'other' }] })
        await expect(controller.revokeOtherSessions(req as never)).resolves.toEqual({ status: 'success', message: 'All other sessions revoked', data: null })
        await expect(controller.revokeSession(req as never, 'other')).resolves.toEqual({ status: 'success', message: 'Session revoked', data: null })
