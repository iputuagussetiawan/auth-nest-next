...
export function MaintenanceTab({ register, setValue, maintenanceMode }: MaintenanceTabProps) {
    return (
        <Card className={maintenanceMode ? 'border-destructive/50 ring-destructive/30 ring-1' : ''}>
            <CardHeader>
                <CardTitle className="flex items-center gap-2 text-base">
                    <TriangleAlert
                        className={`h-4 w-4 ${maintenanceMode ? 'text-destructive' : ''}`}
                    />
                    Maintenance Mode
                </CardTitle>
                <CardDescription>
            ...
            <CardContent className="space-y-4">
                ...
                />
                {maintenanceMode && (
                    <div className="border-destructive/20 bg-destructive/10 rounded-lg border px-4 py-3">
