    ...
    const filtered = useMemo(
        () =>
            users.filter((u) => {
                ...
                if (filters.status !== ALL && u.isActive !== (filters.status === 'active'))
                    return false
                if (
                    filters.verified !== ALL &&
                    u.isEmailVerified !== (filters.verified === 'verified')
                )
                    return false
                return true
            }),
    ...
    return (
        <DashboardPageMain>
            ...
            <DashboardPageCard className="overflow-visible">
                <div className="mb-3 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex min-w-0 flex-1 items-center gap-3">
                        ...
                        <span className="text-muted-foreground shrink-0 text-sm">
                            {searchedUsers.length} of {users.length} users
                        </span>
                    </div>
                    <UsersToolbar
                ...
                <div className="mt-4">
                    {view === 'flat' ? (
                        <div className="bg-card rounded-[24px]">
                            <UiDataTable
                                columns={columns}
                                data={searchedUsers}
                                hideSearch
